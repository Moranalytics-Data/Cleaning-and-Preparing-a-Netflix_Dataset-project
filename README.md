...
While reviewing the various columns to ensure that each contained the expected type of data, I discovered a small error in the original dataset: a smattering of titles whose running time was listed in the `ratin`g field, instead of `duration`.
...
![Actors_Collaborations](IMAGES/Actors Collaborations _ gray.png)
